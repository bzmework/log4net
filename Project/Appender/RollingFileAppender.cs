...
namespace log4net.Appender
{
	...
	public class RollingFileAppender : FileAppender
	{
		...
		private void InitializeRollBackups(string baseFile, ArrayList arrayFiles)
		{
			if (arrayFiles != null)
			{
				string baseFile2 = CompatibilityExtensions.ToLower(baseFile, CultureInfo.InvariantCulture);
				foreach (string arrayFile in arrayFiles)
				{
					InitializeFromOneFile(baseFile2, CompatibilityExtensions.ToLower(arrayFile, CultureInfo.InvariantCulture));
				}
			}
		}
		...
		{
			DateTime dateTime = s_date1970;
			string text = dateTime.ToString(datePattern, DateTimeFormatInfo.InvariantInfo);
			for (int i = 0; i <= 5; i++)
			{
		...
		public override void ActivateOptions()
		{
			...
			m_mutexForRolling = new Mutex(initiallyOwned: false, m_baseFileName.Replace("\\", "_").Replace(":", "_").Replace("/", "_"));
